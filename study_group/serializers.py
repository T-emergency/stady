...
class StudyAuthorSerializer(serializers.ModelSerializer):
    ...
    def get_is_student(self, obj):
        is_student = obj.student_set.filter(user=self.context.get('user'), post =obj, is_accept = None).exists()
    ...
    def get_sended(self, obj):
        sended = obj.student_set.filter(user=self.context.get('user'), post =obj, is_accept = True).exists()
        print("신청자:",sended)
        return sended
    ...
    def get_submit(self, obj):
        if self.get_is_author(obj):
            submit = [{"id": submit.id, "user": submit.user.username, "is_accept":submit.is_accept,} for submit in obj.submit.order_by()]
            # print("dir:",dir(obj.submit))
            # print("submit:",obj.submit.order_by())
            print("submit:",submit)
        else :
            submit_data = obj.submit.filter(user=self.context.get('user'))
            submit = [{"id": submit.id, "user": submit.user.username, "is_accept":submit.is_accept,} for submit in submit_data]
