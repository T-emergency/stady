...
class Study(models.Model):
    ...
    # None 0 1 로 구분이 안된다면 CharField로 구분 # TODO is_on_line으로 변수명 변경
    on_off_line = models.BooleanField(default=False)
    ...
    tags = models.ManyToManyField(
        Tag, related_name='tags', null=True, blank=True)
    # 참여자와 연동되는 이유는 여러 유저를 참조하는 것은 맞으나 is_accept를 사용하기 위해서
    submit = models.ManyToManyField(
        "Student", blank=True, related_name='submiter')
